...
- **Practical chat features**: Message edit/delete, burn conversation, and theme customization.
...
- **Forward Secrecy Improvements**: Compatible clients rotate live session transport keys during a conversation, reducing the value of any single long-lived session key if it were ever exposed later.
...
- **Reliable message controls**: Delivery indicators, editing, deletion, and burn actions.
...
### 5. Start Chatting
Select a contact, type, send. Edit or delete messages and burn conversations. Connection happens automatically - direct if possible, relay as fallback.
...
A relayed connection is functionally identical to a direct one from a privacy standpoint. The relay only handles the underlying transport - it is provably blind to all plaintext, and compatible clients still perform the same in-session key rotation over that relayed path.
...
Setting up a relay is free - the relay server is included with Zer0Talk at no extra software cost.
...
- **Threat Model**: See [THREAT-MODEL.md](THREAT-MODEL.md) for explicit security assumptions, limitations, and non-goals.
...
**Ready to go? Download from [Releases](https://github.com/AnotherLaughingMan/Zer0Talk-Releases/releases/latest) and connect freely - no age-verification theater, just direct contact-to-contact verification.**
