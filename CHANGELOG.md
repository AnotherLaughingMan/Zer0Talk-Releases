...
### Added
- **WAN relay autodiscovery via federation gossip** (`RELAY-MESH-PEERS`): relay servers can now discover each other dynamically without static configuration. After a successful `RELAY-HELLO` handshake, the relay fetches the new peer's known relay list via `RELAY-MESH-PEERS` and auto-connects to any unknown addresses (rate-limited: 1 new connection per 2 s, capped by `MaxFederationPeers`). A background gossip task then re-fetches the mesh peer list from every healthy peer every 5 minutes, letting the federation mesh grow and heal automatically. Opt-out via `"MeshDiscoveryEnabled": false` in `relay-config.json`.
