...
- **Client-side hosted server services (Phase 7)**: three new client services wire the app to the hosted server mode.
  - `ServerAccountService`: persistent TCP connection to the home server; Ed25519 challenge-response account registration (`ACCOUNT-REG`) with DPAPI-protected token cache; transparent token re-auth on reconnect (`ACCOUNT-AUTH`); background receive loop routing incoming server push lines via `PushReceived` event; `SendCommandAsync` for request-response command pairs; `SendFireAndForgetAsync` for one-way sends; `ConnectionLost` event for reconnect coordination.
  - `RoomKeyStore`: DPAPI-encrypted per-room symmetric key journal (`room-keys.dpapi`); lazy-loaded in-memory cache with thread safety; `GetKey`, `SetKey`, `DeleteKey`, `Clear`, `HasKey` API.
  - `RoomService`: full room lifecycle commands (`CreateRoomAsync`, `InviteAsync`, `JoinAsync`, `LeaveAsync`, `GetMembersAsync`, `KickAsync`, `BanAsync`, `TransferAdminAsync`, `SendServerMessageAsync`); hybrid routing awareness — tracks per-room admin relay key for P2P mode vs. server-fan-out mode; incoming push dispatcher routes `ROOM-INVITED`, `ROOM-ADMIN-ONLINE/OFFLINE`, `ROOM-DELIVER`, `ROOM-MEMBER-JOINED/LEFT`, `ROOM-REKEY`, `ROOM-AWAITING-KEY` to ViewModel-bindable events; auto-clears admin-online state on connection loss.
  - All three services registered in `AppServices`.
  - `AppSettings` extended with `HomeServer`, `HomeServerTokenProtected`, `SavedRoomServers`, and `JoinedRoomIds` fields.
- **Rooms window UI (Phase 8)**: full room management UI accessible from the NavRail and the settings context menu.
  - `RoomsWindow` (`Views/RoomsWindow.axaml`) — 920×580 standalone singleton window with custom chrome. Left panel (260 px): live room list with admin-online status indicator and create-room form (name + member cap). Right panel: room header (ID, status, Refresh / Leave / Copy ID), scrollable member list with UID pills + Admin/Mod role badges + cross-server home-server labels, per-selection Kick and Ban action bar, and an inline Invite form supporting optional cross-server routing.
  - `RoomsViewModel` (`ViewModels/RoomsViewModel.cs`) — server connection (Connect / Disconnect with 15 s timeout, status text, connected indicator); room CRUD via `RoomService`; member list refresh on selection change and on `MemberJoined`/`MemberLeft` push events; admin-online state mirrored onto each `RoomEntry`; all destructive actions guarded by `ConfirmDestructiveAsync` dialogs.
  - NavRail: new "People" icon button (&#xE716;) added to the primary navigation rail that opens `RoomsWindow`.
  - Settings gear context menu: "Rooms & Server" entry added.
  - `MainWindow`: subscribes to `AppServices.Rooms.RoomInviteReceived` and shows an actionable `ConfirmAsync` dialog letting users open the Rooms window immediately to accept.
- **S2S room federation (Phase 6)**: `RoomFederationManager` establishes persistent, HMAC-authenticated TCP connections between hosted servers on `HostingS2SPort` (default 8445). Room events (`ROOM-ADMIN-ONLINE`, `ROOM-MEMBER-JOINED`, `ROOM-REKEY`, etc.) are now routed via S2S to members on peer servers. Room invites can be forwarded cross-server via S2S (`ROOM-INVITE <roomId> <targetUid> <targetHomeServer>`). Inbound S2S delivers to locally-connected sessions or queues invites for offline delivery. Added `PeerHostedServers`, `HostingS2SPort`, and `HostingAddress` to `RelayConfig`; `RoomFederationManager` is wired into `RelayHost` Start/Stop alongside `HostedServerHost`.
- **Relay hosted server mode (Phases 0–1 foundation)**: relay binary can now optionally run as a hosted server (`EnableHosting = true`). Added 7 new config fields (`HostingPort`, `MaxRegisteredUsers`, `MaxRoomsPerUser`, `MaxMembersPerRoom`, `RoomMessageQueueDepth`, `DataDirectory`), `Microsoft.Data.Sqlite` + `Sodium.Core` packages, and a full SQLite database layer (`ServerDatabase`, `UserRepository`, `RoomRepository`). Ed25519 challenge-response account registration (`ACCOUNT-REG` / `ACCOUNT-AUTH`), room lifecycle protocol (`ROOM-CREATE` / `ROOM-INVITE` / `ROOM-JOIN` / `ROOM-LEAVE` / `ROOM-KICK` / `ROOM-BAN` / `ROOM-TRANSFER-ADMIN` / `ROOM-MEMBERS`), hybrid offline-message routing (`ROOM-MSG` fans out ciphertext when admin is offline), group rekey signals (`ROOM-REKEY` / `ROOM-AWAITING-KEY`), and a thread-safe message ring buffer (`RoomMessageQueue`) are implemented. Hosted server lifecycle is integrated into `RelayHost` (`Start`/`Stop`) behind the feature flag.
...
- **Hosted server restart leaves stale auth/room state**: `HostedServerHost.Stop()` now clears authenticated sessions, message queues, admin-online state, and pending invites so the server restarts with a clean slate.
- **S2S `HostingAddress` missing port**: relay now appends `:{HostingS2SPort}` to `HostingAddress` when the address does not already contain a colon, ensuring peer servers receive a valid `host:port` to route federated messages back to this server.
- **`relay-config-guide.md` incomplete**: guide now documents all current config fields including hosting fields (`EnableHosting`, `HostingPort`, `HostingS2SPort`, `HostingAddress`, `PeerHostedServers`, `MaxRegisteredUsers`, `MaxRoomsPerUser`, `MaxMembersPerRoom`, `RoomMessageQueueDepth`, `DataDirectory`), previously undocumented fields (`ExposeSensitiveClientData`, `OperatorBlockSeconds`, `EnableSmoothScrolling`, `MaxFederationPeers`, `FederationSyncIntervalSeconds`), and a full Troubleshooting section covering public-IP connectivity, LAN-only failures, and S2S federation issues.
- **Rooms — Create Room / Invite / all commands silently failed**: `ServerAccountService.SendCommandAsync` was reading the server response directly from the `NetworkStream` while `ReceiveLoopAsync` was also reading from the same stream concurrently. The receive loop won the race and swallowed the `OK` response, so `SendCommandAsync` returned `null` for every command and all room operations appeared to do nothing. Fixed by removing the direct stream read from `SendCommandAsync`: it now sets a `TaskCompletionSource<string?>` (`_pendingCmdTcs`) before writing the command; `ReceiveLoopAsync` checks for a pending TCS on every line it reads and routes command responses back to the caller via `TrySetResult`, routing all other lines to `PushReceived` as before. A dedicated `_cmdLock` serializes concurrent command calls. `Disconnect()` calls `TrySetResult(null)` on any pending TCS so callers don't hang on connection loss.
