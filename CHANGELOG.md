...
- **New built-in themes**: added `Monokai Dimmed`, `High Contrast`, `High Contrast Dark`, `High Visibility`, and `Mercer Blue` (inspired by `#778B8F`) as built-in selectable themes in Settings > Appearance.
...
### Updated
- **Markdown temporarily removed**: markdown support has been removed for the time being.
- **Settings > Performance > CPU CCD gating and messaging**: CCD switching is now disabled unless the CPU is a supported dual-CCD AMD X3D configuration. Intel CPUs now show an explicit notice that Windows scheduling already handles core placement, single-CCD CPUs remain disabled, and dual-CCD AMD CPUs without X3D now explicitly report that no X3D CCD was detected and pinning is not beneficial for Zer0Talk's encryption/messaging workload profile.
...
- **Contacts UI/workflow refresh**: moved add-contact affordance into nav rail, improved contact empty states, and introduced local contact recovery action in the contacts panel.
- **Theme/readability tuning for contacts**: added per-theme `App.ContactOfflineOpacity` resources and presence-to-opacity conversion for clearer offline contact presentation.
- **Lock/logout behavior**: explicit logout path now clears remembered passphrase preference/secret before locking.
- **Auto-update UX flow**: update checks now queue persistent actionable notices and support install-now vs postpone decisions with configurable postpone hours.
- **Settings backup/update controls**: added General settings controls for contact backup retention and update postpone interval.
- **Unread delta fan-out through EventHub**: app event wiring now forwards per-peer unread count changes (`UnreadCountChanged`) into `EventHub` for transport adapters and listeners.
...
- **Timeout/cancellation consistency**: tightened cancellation propagation and timeout handling in verification/update and relay-adjacent call paths.
