        ...
        private void Evaluate()
        {
                ...
                else if (!inStartupGrace)
                {
                    ...
                    if (_consecutiveNetFailures >= netFailThreshold && pastObservationWindow && !postRestartSuppressed)
                    {
                        ...
                        else if (_network.DiscoveryBehavior != NetworkService.DiscoveryMode.BroadcastOnly)
                        {
                            ...
                            try { P2PTalk.Utilities.ErrorLogger.LogException(new InvalidOperationException("[Regression] Discovery mode override → BroadcastOnly (network anomalies)"), source: "Regression.Network"); } catch { }
                    ...
                    if (_consecutiveEncFailures >= 2)
                    {
                        ...
                        try { P2PTalk.Utilities.ErrorLogger.LogException(new InvalidOperationException("[Regression] Encryption self-test failed repeatedly"), source: "Regression.Crypto"); } catch { }
            ...
            if (_consecutiveDiscFailures >= discFailThreshold)
                    {
                        ...
                        try { AppServices.Discovery.Restart(); Logger.Log("RegressionGuard: restarted DiscoveryService due to anomalies."); try { P2PTalk.Utilities.ErrorLogger.LogException(new InvalidOperationException("[Regression] DiscoveryService restarted due to anomalies"), source: "Regression.Discovery"); } catch { } } catch { }
            ...
            if (_consecutiveDiscFailures >= discFailThreshold * 2)
                ...
                MaybeRestoreCheckpoint();
                            ...
                            try { P2PTalk.Utilities.ErrorLogger.LogException(new InvalidOperationException("[Regression] Restored last-known-good checkpoint due to discovery failures"), source: "Regression.Discovery"); } catch { }
    ...
    private int GetDiscoveryFailureThreshold()
    {
        ...
        private void TryRestartNetwork()
        {
            try
            {
                ...
                try { P2PTalk.Utilities.ErrorLogger.LogException(new InvalidOperationException("[Regression] Network restart to recover from anomalies"), source: "Regression.Network"); } catch { }
            }
    ...
    private TimeSpan GetPostRestartSuppressionWindow()
    {
        ...
        private void SaveCheckpoint()
        {
            try
            {
                ...
                P2PTalk.Services.DiscoveryService.Snapshot? ds = null;
                ...
                var cp = new Checkpoint
                {
                    ...
                    DiscoveryState = ds?.StateValue ?? P2PTalk.Services.DiscoveryService.State.Idle,
        ...
        private void RestoreCheckpoint()
        {
            try
            {
                ...
                if (changed)
                {
                    ...
                    Logger.Log(msg);
                    try { AppServices.Events.RaiseRegressionDetected(msg); } catch { }
                    try { P2PTalk.Utilities.ErrorLogger.LogException(new InvalidOperationException("[Regression] Checkpoint restore applied (port/major/discovery)"), source: "Regression.Network"); } catch { }
                    TryRestartNetwork();
                }
                ...
                try
                {
                    ...
                    if (cp.DiscoveryState == P2PTalk.Services.DiscoveryService.State.Completed && snap.StateValue != P2PTalk.Services.DiscoveryService.State.Completed)
                    {
        ...
        private sealed class Checkpoint
        {
            ...
            public P2PTalk.Services.DiscoveryService.State DiscoveryState { get; set; } = P2PTalk.Services.DiscoveryService.State.Idle;
        ...
        private bool EvaluateDiscovery()
        {
            try
            {
                ...
                if (snap.StateValue == P2PTalk.Services.DiscoveryService.State.Discovering && snap.LastAttemptUtc is DateTime la && (DateTime.UtcNow - la) > TimeSpan.FromSeconds(75))
                {
                ...
                // Healthy signals
                if (snap.PeersCount > 0 || snap.UdpBeaconsRecv > 0 || snap.StateValue == P2PTalk.Services.DiscoveryService.State.Completed)
                {
