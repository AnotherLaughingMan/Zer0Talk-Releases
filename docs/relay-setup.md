...
## Overview
The relay server is a separate Zer0Talk executable that forwards encrypted peer traffic and helps peers behind strict NAT connect. It does not store message content or user private keys.

Relay usage is fallback-oriented: clients attempt direct/NAT traversal first, then use relay when needed. This keeps Zer0Talk decentralized while improving reliability on restrictive networks.

Relays can also be operated in federated topologies (multiple independent relay operators), so deployments do not depend on a single central relay authority.
...
- Relay can see connection metadata needed for routing (for example, endpoint/session coordination).
- Relay does **not** decrypt chat payloads and does not persist message history.
- Client identity keys remain client-side; private keys are never uploaded to relay.
